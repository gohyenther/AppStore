        ...
        if request.POST['action'] == 'office_rent':
            with connection.cursor() as cursor:
                ...
                if checkRent != None:
                    ...
                    cursor.execute("INSERT INTO transaction VALUES(%s, %s, %s)", [id, start_rent, rate])
    ## Rent storage space
    if request.POST:
        if request.POST['action'] == 'storage_rent':
            with connection.cursor() as cursor:
                ...
                timescale = cursor.fetchone()
                cursor.execute("SELECT rate FROM storages WHERE unit = %s AND street = %s AND unit_no = %s AND postal_code = %s",
                               [request.POST['store_unit'], request.POST['store_street'], request.POST['store_unit_no'], request.POST['store_postal_code']])
                rate = cursor.fetchone()
                cursor.execute("SELECT * FROM rent WHERE unit = %s AND street = %s AND unit_no = %s AND postal_code = %s",
                               [request.POST['store_unit'], request.POST['store_street'], request.POST['store_unit_no'], request.POST['store_postal_code']])
    ...
    if request.POST:
        if request.POST['action'] == 'confroom_rent':
            with connection.cursor() as cursor:
                ...
                timescale = cursor.fetchone()
                cursor.execute("SELECT rate FROM confrooms WHERE unit = %s AND street = %s AND unit_no = %s AND postal_code = %s",
                               [request.POST['conf_unit'], request.POST['conf_street'], request.POST['conf_unit_no'], request.POST['conf_postal_code']])
                rate = cursor.fetchone()
                cursor.execute("SELECT * FROM rent WHERE unit = %s AND street = %s AND unit_no = %s AND postal_code = %s",
                               [request.POST['conf_unit'], request.POST['conf_street'], request.POST['conf_unit_no'], request.POST['conf_postal_code']])
    ...
    if request.POST:
        if request.POST['action'] == 'workcube_rent':
            with connection.cursor() as cursor:
                ...
                timescale = cursor.fetchone()
                cursor.execute("SELECT rate FROM workcubes WHERE unit = %s AND street = %s AND unit_no = %s AND postal_code = %s",
                               [request.POST['cube_unit'], request.POST['cube_street'], request.POST['cube_unit_no'], request.POST['cube_postal_code']])
                rate = cursor.fetchone()
                cursor.execute("SELECT * FROM rent WHERE unit = %s AND street = %s AND unit_no = %s AND postal_code = %s",
                               [request.POST['cube_unit'], request.POST['cube_street'], request.POST['cube_unit_no'], request.POST['cube_postal_code']])
...
def adminanalytics(request):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("SELECT t.amount_paid, r.unit FROM transaction t, rent r WHERE t.customerid = r.customerid AND t.transactionid = r.start_rent GROUP BY r.unit, t.amount_paid")
